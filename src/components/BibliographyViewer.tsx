...
export function BibliographyViewer({ bibliography }: BibliographyViewerProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs sm:text-sm text-stone-500 dark:text-stone-400 pb-3 border-b border-stone-200 dark:border-stone-800">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-semibold bg-stone-100 hover:bg-stone-200 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-200 transition"
          >
      ...
      <header className="space-y-4">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs sm:text-sm font-semibold bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-300/60 dark:border-amber-700/50">
        ...
        <p className={`text-stone-600 dark:text-stone-300 ${contentFontClass} max-w-3xl`}>
          {bibliography.description}
        </p>

        <div className="flex flex-wrap items-center gap-2 pt-2 text-xs sm:text-sm">
          <span className="px-3 py-1.5 rounded-lg bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 font-medium">
            共收錄 <strong className="text-amber-700 dark:text-amber-400 font-bold">{bibliography.totalBooks}</strong> 部權威註釋書與專題著作
          </span>
          <span className="px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 border border-amber-200/60 dark:border-amber-800/40">
      ...
      <div className="space-y-4 pt-2">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          {/* Category Tabs */}
          <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-stone-100 dark:bg-stone-900 border border-stone-200 dark:border-stone-800 overflow-x-auto text-xs sm:text-sm font-semibold">
            <button
          ...
          {/* Quick Actions */}
          <div className="flex items-center gap-2 self-end md:self-auto text-xs sm:text-sm text-stone-500 dark:text-stone-400">
            <button
              onClick={expandAll}
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-white dark:bg-stone-950 border border-stone-200 dark:border-stone-800 text-sm sm:text-base text-stone-800 dark:text-stone-200 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-amber-500 transition shadow-2xs"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs sm:text-sm text-stone-400 hover:text-stone-600 dark:hover:text-stone-200"
            >
              清除
            </button>
          )}
      ...
      <div className="space-y-6">
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 bg-stone-50 dark:bg-stone-900/40 rounded-3xl border border-dashed border-stone-300 dark:border-stone-800 space-y-3">
            ...
            <p className="text-stone-600 dark:text-stone-400 font-medium text-sm sm:text-base">
            ...
            <button
              ...
              className="text-xs sm:text-sm text-amber-600 dark:text-amber-400 underline font-medium"
            >
              清除篩選條件
            </button>
          </div>
          ...
          filteredItems.map((item) => {
            ...
            const categoryStyle = {
              ...
              pastoral: {
                ...
                icon: <Compass className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />,
              },
              academic: {
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-stone-100 dark:border-stone-800/80 pb-3.5">
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="px-2.5 py-0.5 rounded-md bg-stone-900 text-white dark:bg-stone-100 dark:text-stone-900 text-xs sm:text-sm font-bold font-mono">
                        #{item.id}
                      </span>
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs sm:text-sm font-semibold border ${categoryStyle.badge}`}
                      >
                        {categoryStyle.icon}
                  ...
                  <button
                    ...
                    className="self-end sm:self-center inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold text-stone-600 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 transition cursor-pointer"
                  >
                ...
                {isExpanded && (
                  <div className="space-y-5 pt-2 animate-in fade-in duration-200">
                    ...
                    {item.authorBackground && (
                      <div className="p-4 sm:p-5 rounded-2xl bg-stone-50 dark:bg-stone-900/60 border border-stone-200/70 dark:border-stone-800/70 space-y-2">
                        <div className={`flex items-center gap-1.5 ${subHeadingFontClass} text-stone-700 dark:text-stone-300`}>
      ...
      <footer className="pt-8 border-t border-stone-200 dark:border-stone-800 flex flex-col sm:flex-row items-center justify-between gap-4">
        <Link
          href="/"
          className="text-xs sm:text-sm font-semibold text-stone-500 hover:text-stone-900 dark:hover:text-stone-100 transition"
        >
          返回靈修推基古首頁
        </Link>
        ...
        <Link
          href="/devotional/youth/2026-09-14/mon"
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-semibold text-sm sm:text-base transition shadow-md shadow-amber-600/20"
        >
